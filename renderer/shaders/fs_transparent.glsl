...
void main() {
    ...
    for (uint i = 0; i < push_constants.light_count; i++) {
        lighting += (light(N, lights_ubo.lights[i].direction, V, lights_ubo.lights[i].color, roughness, albedo, metallic) * lights_ubo.lights[i].intensity * occlusion);
    }

    // todo: toto je zle, lebo to zoslabi aj odlesky svetla, ktore by nemali byt priesvitne
    hdr = vec4(lighting, opacity);
}